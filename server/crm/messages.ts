...
import { normalizePhone, sendSms, smsMissingEnv, resolveSmsSender, orgCanTextClients, CLIENT_TEXT_NEEDS_OWN_NUMBER } from "./sms";
...
export function registerCrmMessageRoutes(app: Express, getDevUser: GetUser): void {
  app.post("/api/crm/messages", async (req: any, res) => {
    ...
    if (channel === "email") {
      ...
      // Client texting needs the org's OWN registered number/account — the
    ...
    try {
      result = await deliverQuickMessage({
        ...
        orgCustomFields: ctx.org.customFields, orgId: ctx.org.id,
      });
    } catch (e: any) {
