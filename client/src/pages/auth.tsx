      ...
      <div className="w-full max-w-md space-y-6">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight" data-testid="text-auth-title">
            {isPortal()
              ? <>ConstructHub <span className="font-extrabold text-primary">CRM</span></>
        ...
        <Card className="p-6 space-y-5" style={{ boxShadow: "var(--shadow-sm)" }}>
          {mode === "login" && (
            <>
              ...
              <a href={nextParam ? `/api/auth/google?next=${encodeURIComponent(nextParam)}` : "/api/auth/google"} data-testid="link-google-login">
          ...
          {mode === "signup" && (
            <>
              ...
              <a href={betaParam ? `/api/auth/google?beta=${encodeURIComponent(betaParam)}` : nextParam ? `/api/auth/google?next=${encodeURIComponent(nextParam)}` : "/api/auth/google"} data-testid="link-google-signup">
                <Button variant="outline" className="w-full gap-2" type="button">
                  <svg className="h-4 w-4" viewBox="0 0 24 24">
                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" />
                    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
